...
<main>
  ...
  <div class="tabs">
    <button id="createTab" class="tab-button active" type="button" onclick="showTab('create')">Create Subliminal</button>
    <button id="analyzeTab" class="tab-button" type="button" onclick="showTab('analyze')">Analyze Audio</button>
  </div>

  <section id="createPanel" class="grid tab-panel active">
    <div class="stack">
      <div class="card">
  ...
  <section id="analyzePanel" class="tab-panel">
    <div class="card">
      <h2>Analyze WAV/MP3 File</h2>
      <p class="mini">
        Drop in a WAV or MP3 to inspect dominant tones, likely solfeggio matches, and possible binaural beat differences.
        Exact subliminal words cannot be reliably decoded from arbitrary mixed audio unless the speech is clear enough to hear.
      </p>
      <div id="analyzerDropZone" class="item">
        <strong>Drop WAV/MP3 here</strong>
        <span class="mini">or choose a file below</span>
      </div>
      <input id="analyzeFileInput" type="file" accept="audio/wav,audio/x-wav,audio/mpeg,audio/mp3" />
      <div id="analysisResults" class="analysis-results">
        <div class="status">Choose a WAV/MP3 file to analyze.</div>
      </div>
    </div>
  </section>
</main>
...
function showTab(tabName) {
  ...
  $("analyzePanel").classList.toggle("active", !isCreate);
  $("createTab").classList.toggle("active", isCreate);
  $("analyzeTab").classList.toggle("active", !isCreate);
}
...
async function analyzeFile(file) {
  ...
  results.innerHTML = `<div class="status">Decoding and analyzing ${escapeHtml(file.name)}...</div>`;

  try {
    const ctx = await getAudioContext();
    const data = await file.arrayBuffer();
    const buffer = await ctx.decodeAudioData(data.slice(0));
    analyzedAudioBuffer = buffer;
    analyzedAudioName = file.name;
    const analysis = analyzeAudioBuffer(buffer, file.name);
    renderAnalysisResults(analysis);
  } catch (error) {
    console.error(error);
    results.innerHTML = `<div class="status">Could not decode that file. Try a standard WAV or MP3.</div>`;
  }
}
...
function analyzeAudioBuffer(buffer, fileName) {
  ...
  const leftPeaks = findDominantPeaks(left, sampleRate);
  const rightPeaks = findDominantPeaks(right, sampleRate);
  const monoPeaks = mergePeaks(leftPeaks, rightPeaks);
  const solfeggioMatches = matchSolfeggio(monoPeaks);
  const binaural = detectBinaural(leftPeaks, rightPeaks);
  const rms = channelRms(left);
...
function findDominantPeaks(channel, sampleRate) {
  const maxSamples = Math.min(channel.length, 32768);
  const start = Math.max(0, Math.floor((channel.length - maxSamples) / 2));
  const segment = channel.subarray(start, start + maxSamples);
  const rawPeaks = [];

  for (let freq = 20; freq <= 1200; freq += 2) {
    const power = goertzelPower(segment, sampleRate, freq);
    rawPeaks.push({freq, power});
  }

  rawPeaks.sort((a, b) => b.power - a.power);
  const peaks = [];
  for (const peak of rawPeaks) {
    if (peaks.every((chosen) => Math.abs(chosen.freq - peak.freq) > 8)) {
      peaks.push(peak);
    }
    if (peaks.length >= 8) break;
  }

  const maxPower = peaks[0]?.power || 1;
  return peaks.map((peak) => ({
    freq: peak.freq,
    strength: peak.power / maxPower
  }));
}
...
function goertzelPower(samples, sampleRate, targetFreq) {
  ...
  for (let i = 0; i < samples.length; i++) {
    const windowed = samples[i] * (0.5 - 0.5 * Math.cos(2 * Math.PI * i / Math.max(1, samples.length - 1)));
...
function mergePeaks(leftPeaks, rightPeaks) {
  const merged = [...leftPeaks, ...rightPeaks].sort((a, b) => b.strength - a.strength);
  const peaks = [];
  for (const peak of merged) {
    if (peaks.every((chosen) => Math.abs(chosen.freq - peak.freq) > 8)) peaks.push(peak);
    if (peaks.length >= 10) break;
  }
  return peaks;
}

function matchSolfeggio(peaks) {
  return solfeggio
    .map((tone) => {
      const nearest = peaks.reduce((best, peak) => {
        const diff = Math.abs(peak.freq - tone.hz);
        return !best || diff < best.diff ? {tone, peak, diff} : best;
      }, null);
      return nearest && nearest.diff <= 4 ? nearest : null;
    })
    .filter(Boolean);
}

function detectBinaural(leftPeaks, rightPeaks) {
  let best = null;
  for (const leftPeak of leftPeaks) {
    for (const rightPeak of rightPeaks) {
      const diff = Math.abs(rightPeak.freq - leftPeak.freq);
      if (diff < 0.5 || diff > 45) continue;
      const score = leftPeak.strength + rightPeak.strength - diff / 100;
      if (!best || score > best.score) {
        best = {
          leftHz: leftPeak.freq,
          rightHz: rightPeak.freq,
          beatHz: diff,
          score,
          band: brainwaveBand(diff)
        };
      }
    }
  }
...
function renderAnalysisResults(analysis) {
  const peakRows = analysis.monoPeaks
    .map((peak) => `<div class="item"><strong>${peak.freq.toFixed(0)} Hz</strong><span class="mini">Relative strength ${(peak.strength * 100).toFixed(0)}%</span></div>`)
    .join("");

  const solfeggioRows = analysis.solfeggioMatches.length
    ? analysis.solfeggioMatches.map((match) => `<div class="item"><strong>${match.tone.title}</strong><span class="mini">Detected near ${match.peak.freq.toFixed(0)} Hz, off by ${match.diff.toFixed(1)} Hz. ${match.tone.desc}</span></div>`).join("")
    : `<div class="item"><strong>No close solfeggio match found</strong><span class="mini">No dominant peak was within 4 Hz of the built-in solfeggio list.</span></div>`;

  const binauralText = analysis.binaural
    ? `<strong>${analysis.binaural.beatHz.toFixed(1)} Hz ${analysis.binaural.band}</strong><span class="mini">Left ${analysis.binaural.leftHz.toFixed(0)} Hz, right ${analysis.binaural.rightHz.toFixed(0)} Hz. This is a likely stereo binaural difference, not proof of intent.</span>`
    : `<strong>No clear binaural beat detected</strong><span class="mini">The analyzer did not find a strong left/right frequency pair between 0.5 and 45 Hz apart.</span>`;
  const defaultEnd = Math.min(analysis.duration, 60).toFixed(1);

  $("analysisResults").innerHTML = `
    <div class="result-grid">
      <div class="item"><span class="mini">Duration</span><span class="result-value">${analysis.duration.toFixed(1)}s</span></div>
      <div class="item"><span class="mini">Channels</span><span class="result-value">${analysis.channels}</span></div>
      <div class="item"><span class="mini">Level RMS</span><span class="result-value">${analysis.rms.toFixed(3)}</span></div>
    </div>

    <div class="card">
      <h3>Dominant Tones</h3>
      <div class="result-grid">${peakRows}</div>
    </div>

    <div class="card">
      <h3>Solfeggio Matches</h3>
      <div class="analysis-results">${solfeggioRows}</div>
    </div>

    <div class="card">
      <h3>Binaural Beat Check</h3>
      <div class="item">${binauralText}</div>
    </div>

    <div class="card">
      <h3>Speech-To-Text Workflow</h3>
      <p class="mini">
        Use this when you need exact words: isolate the most speech-like part, preview it, export a speech-focused WAV,
        then send that WAV to a speech-to-text service or keep notes here while listening.
      </p>
      <div class="row">
        <label>Start second
          <input id="sttStart" type="number" min="0" max="${analysis.duration.toFixed(1)}" value="0" step="0.1" />
        </label>
        <label>End second
          <input id="sttEnd" type="number" min="0.1" max="${analysis.duration.toFixed(1)}" value="${defaultEnd}" step="0.1" />
        </label>
      </div>
      <div class="row">
        <label>Isolation preset
          <select id="sttMode">
            <option value="speech-band">Speech band mono, 120-4000 Hz</option>
            <option value="center">Center vocal extraction</option>
            <option value="mono">Mono full-range</option>
          </select>
        </label>
        <label>Preview gain
          <input id="sttGain" type="range" min="-24" max="18" value="6" step="1" />
      ...
      <label>Transcript notes
        <textarea id="sttTranscript" placeholder="Paste or type the transcript here after reviewing the isolated file."></textarea>
      </label>
      <div id="sttStatus" class="status">Ready to isolate a speech-focused segment from ${escapeHtml(analysis.fileName)}.</div>
    </div>

    <div class="card">
      <h3>Subliminal Message Decoding</h3>
      <p class="note">
        Exact words cannot be reliably deciphered from arbitrary mixed subliminal audio in the browser. This analyzer can tell you
        what tones and binaural differences are present. To know exactly what words are being said, use the original project,
        audible voice recordings, or a dedicated speech-to-text workflow on a clearly isolated vocal track.
      </p>
    </div>
  `;
}

function speechWorkflowSettings() {
  if (!analyzedAudioBuffer) {
    return null;
  }
  const duration = analyzedAudioBuffer.duration;
  const start = clamp(Number($("sttStart")?.value || 0), 0, Math.max(0, duration - 0.1));
  const end = clamp(Number($("sttEnd")?.value || duration), start + 0.1, duration);
  const mode = $("sttMode")?.value || "speech-band";
  const gain = dbToGain(Number($("sttGain")?.value || 0));
  return {start, end, mode, gain};
}

function buildSpeechWorkflowBuffer(ctx) {
  const settings = speechWorkflowSettings();
  if (!settings) return null;

  const source = analyzedAudioBuffer;
  const sr = source.sampleRate;
  const startFrame = Math.floor(settings.start * sr);
  const endFrame = Math.floor(settings.end * sr);
  const length = Math.max(1, endFrame - startFrame);
  const output = ctx.createBuffer(1, length, sr);
  ...
  for (let i = 0; i < length; i++) {
    ...
    const mono = settings.mode === "center"
  ...
  if (settings.mode === "speech-band") {
    applyHighPass(out, sr, 120);
    applyLowPass(out, sr, 4000);
  }

  for (let i = 0; i < out.length; i++) {
    out[i] = clamp(out[i] * settings.gain, -1, 1);
  }

  return output;
}

function applyHighPass(samples, sampleRate, cutoff) {
...
function applyLowPass(samples, sampleRate, cutoff) {
  const dt = 1 / sampleRate;
  const rc = 1 / (2 * Math.PI * cutoff);
...
async function previewSpeechWorkflow() {
  stopPreview();
  const ctx = await getAudioContext();
  const buffer = buildSpeechWorkflowBuffer(ctx);
  if (!buffer) {
    $("sttStatus").textContent = "Analyze a WAV/MP3 first, then preview the isolated speech segment.";
    return;
  }
  const src = ctx.createBufferSource();
  src.buffer = buffer;
...
async function exportSpeechWorkflow() {
  const ctx = await getAudioContext();
  const buffer = buildSpeechWorkflowBuffer(ctx);
  if (!buffer) {
    $("sttStatus").textContent = "Analyze a WAV/MP3 first, then export the isolated speech segment.";
    return;
  }
  ...
  const base = (analyzedAudioName || "analyzed-audio").replace(/\.[^.]+$/, "").replace(/[^a-z0-9_-]+/gi, "-");
  link.href = url;
  link.download = `${base}-speech-isolated.wav`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  window.setTimeout(() => URL.revokeObjectURL(url), 30000);
  $("sttStatus").textContent = "Exported a speech-focused WAV. Use that file with a speech-to-text service for the best chance of exact wording.";
}

async function copySpeechWorkflowNotes() {
  const settings = speechWorkflowSettings();
  const transcript = $("sttTranscript")?.value.trim() || "";
  const notes = [
    `File: ${analyzedAudioName || "No file analyzed"}`,
    settings ? `Segment: ${settings.start.toFixed(1)}s to ${settings.end.toFixed(1)}s` : "Segment: none",
    settings ? `Isolation: ${settings.mode}` : "Isolation: none",
    "",
    "Transcript notes:",
    transcript || "(empty)"
  ].join("\n");
  ...
  try {
    ...
    $("sttStatus").textContent = "Copied workflow notes to clipboard.";
  } catch (_) {
    $("sttStatus").textContent = notes;
  }
}
...
$("analyzeFileInput").addEventListener("change", handleAnalyzeFile);
$("analyzerDropZone").addEventListener("dragover", (event) => {
  event.preventDefault();
  $("analyzerDropZone").style.borderColor = "rgba(125, 211, 252, 0.7)";
});
...
html(APP_HTML, height=2200, scrolling=True)
